...
fn make_product(price: f64) -> Product {
    Product {
...
fn no_discount_returns_base_price() {
    let p = make_product(10.0);
    assert_eq!(effective_unit_price(&p, 1.0), 10.0);
    assert_eq!(effective_unit_price(&p, 100.0), 10.0);
}

#[test]
fn fixed_discount_subtracts_value() {
    let mut p = make_product(10.0);
    p.discount_kind = Some("fixed".into());
    p.discount_value = Some(3.0);
    assert!((effective_unit_price(&p, 1.0) - 7.0).abs() < 0.001);
}

#[test]
fn fixed_discount_never_below_zero() {
    let mut p = make_product(5.0);
    p.discount_kind = Some("fixed".into());
    p.discount_value = Some(20.0); // desconto > preço
    assert_eq!(effective_unit_price(&p, 1.0), 0.0);
}

#[test]
fn percent_discount_applies_correctly() {
    let mut p = make_product(100.0);
    p.discount_kind = Some("percent".into());
    p.discount_value = Some(20.0);
    assert!((effective_unit_price(&p, 1.0) - 80.0).abs() < 0.001);
}
...
fn bulk_fixed_single_tier_not_triggered_below_min() {
    ...
    p.discount_value = Some(2.0);
    p.discount_min_qty = Some(5.0);
    assert_eq!(effective_unit_price(&p, 4.0), 10.0); // abaixo do gatilho
    assert!((effective_unit_price(&p, 5.0) - 8.0).abs() < 0.001); // exatamente
    assert!((effective_unit_price(&p, 10.0) - 8.0).abs() < 0.001); // acima
}
...
fn bulk_percent_single_tier() {
    ...
    p.discount_value = Some(10.0);
    p.discount_min_qty = Some(3.0);
    assert_eq!(effective_unit_price(&p, 2.0), 100.0);
    assert!((effective_unit_price(&p, 3.0) - 90.0).abs() < 0.001);
}
...
fn bulk_fixed_multi_tiers_pick_highest_satisfied() {
    ...
    assert_eq!(effective_unit_price(&p, 1.0), 10.0);
    assert!((effective_unit_price(&p, 2.0) - 9.0).abs() < 0.001);
    assert!((effective_unit_price(&p, 4.0) - 9.0).abs() < 0.001);
    assert!((effective_unit_price(&p, 5.0) - 8.0).abs() < 0.001);
    assert!((effective_unit_price(&p, 9.0) - 8.0).abs() < 0.001);
    assert!((effective_unit_price(&p, 10.0) - 6.0).abs() < 0.001);
    assert!((effective_unit_price(&p, 50.0) - 6.0).abs() < 0.001);
}
...
fn bulk_tiers_in_unsorted_order_still_picks_highest() {
    ...
    assert!((effective_unit_price(&p, 6.0) - 8.0).abs() < 0.001);
}
