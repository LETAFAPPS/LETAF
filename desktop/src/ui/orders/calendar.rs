...
fn cal_rebuild(ui: &MainWindow) {
    ...
    let mut y = ui.global::<OrdersState>().get_cal_year();
    let mut m = ui.global::<OrdersState>().get_cal_month();

    let start = parse_ymd(ui.global::<OrdersState>().get_order_date_start().as_ref());
    let end = parse_ymd(ui.global::<OrdersState>().get_order_date_end().as_ref());
    ...
    if y == 0 || !(1..=12).contains(&m) {
        ...
        y = base.year();
        m = base.month() as i32;
        ui.global::<OrdersState>().set_cal_year(y);
        ui.global::<OrdersState>().set_cal_month(m);
    }
    ...
    ui.global::<OrdersState>().set_cal_days(ModelRc::new(VecModel::from(days)));
    ui.global::<OrdersState>().set_cal_title(SharedString::from(format!("{} {}", month_pt(m), y)));
    ui.global::<OrdersState>().set_order_date_label(SharedString::from(range_label(start, end)));
}
...
pub(crate) fn setup_calendar(ui: &MainWindow) {
    ...
    {
        let w = ui.as_weak();
        ui.global::<OrdersState>().on_cal_toggle(move || {
            let Some(ui) = w.upgrade() else { return };
            let open = !ui.global::<OrdersState>().get_cal_open();
            ui.global::<OrdersState>().set_cal_open(open);
            if open {
                cal_rebuild(&ui);
            }
        });
    ...
    {
        let w = ui.as_weak();
        ui.global::<OrdersState>().on_cal_prev(move || {
            let Some(ui) = w.upgrade() else { return };
            let mut y = ui.global::<OrdersState>().get_cal_year();
            let mut m = ui.global::<OrdersState>().get_cal_month() - 1;
            if m < 1 {
                m = 12;
                y -= 1;
            }
    ...
    {
        let w = ui.as_weak();
        ui.global::<OrdersState>().on_cal_next(move || {
            let Some(ui) = w.upgrade() else { return };
            let mut y = ui.global::<OrdersState>().get_cal_year();
            let mut m = ui.global::<OrdersState>().get_cal_month() + 1;
            if m > 12 {
    ...
    {
        let w = ui.as_weak();
        ui.global::<OrdersState>().on_cal_pick(move |ymd| {
            let Some(ui) = w.upgrade() else { return };
            ...
            if start.is_none() || end.is_some() {
                ...
                ui.global::<OrdersState>().set_order_date_start(ymd.clone());
                ui.global::<OrdersState>().set_order_date_end(SharedString::default());
                ...
                if picked < s {
                    ...
                    ui.global::<OrdersState>().set_order_date_end(ymd.clone());
                }
            }
            cal_rebuild(&ui);
    ...
    {
        let w = ui.as_weak();
        ui.global::<OrdersState>().on_cal_clear(move || {
            let Some(ui) = w.upgrade() else { return };
            ui.global::<OrdersState>().set_order_date_start(SharedString::default());
            ui.global::<OrdersState>().set_order_date_end(SharedString::default());
            cal_rebuild(&ui);
            ui.global::<OrdersState>().invoke_refresh_orders();
        });
...
pub(crate) fn setup_refresh_order_elapsed(ui: &MainWindow) {
    ...
    ui.global::<OrdersState>().on_refresh_order_elapsed(move || {
        let Some(ui) = ui_weak.upgrade() else { return };
        let mut current = ui.global::<OrdersState>().get_detail_order();
        ...
        if new_elapsed != current.elapsed_display.as_str() {
            ...
            ui.global::<OrdersState>().set_detail_order(current);
        }
    });
}
