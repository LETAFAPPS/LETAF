...
pub(crate) fn setup_save_addon(
    ...
    ui.global::<AddonsState>().on_save_addon(move || {
        let Some(ui_ref) = ui_weak.upgrade() else { return };
        let group_id_str = ui_ref.global::<AddonsState>().get_selected_addon_group_id().to_string();
        let Ok(group_id) = Uuid::parse_str(&group_id_str) else { return };
        let id_str = ui_ref.global::<AddonsState>().get_addon_form_id().to_string();
        let name = ui_ref.global::<AddonsState>().get_addon_form_name().trim().to_string();
        if name.is_empty() {
            ui_ref.global::<AddonsState>().set_addon_form_error(SharedString::from("Preencha o nome do adicional"));
            return;
        }
        let price = parse_decimal(&ui_ref.global::<AddonsState>().get_addon_form_price()).unwrap_or(0.0);
        ui_ref.global::<AddonsState>().set_addon_form_error(SharedString::default());
            ...
            let _ = slint::invoke_from_event_loop(move || {
                ...
                match res {
                    Ok(()) => {
                        ...
                        ui.global::<AddonsState>().invoke_select_addon_group(group_id_ss);
                    }
                    Err(e) => {
                        let msg = friendly_error(&e);
                        show_toast(&ui, &msg, "error");
                        ui.global::<AddonsState>().set_addon_form_error(SharedString::from(msg));
                    }
                }
            });
        });
...
pub(crate) fn setup_delete_addon(
    ...
    ui.global::<AddonsState>().on_delete_addon(move |id_str| {
...
pub(crate) fn setup_toggle_addon_active(
    ...
    ui.global::<AddonsState>().on_toggle_addon_active(move |id_str| {
        let Ok(id) = Uuid::parse_str(id_str.as_str()) else { return };
        let ui_weak = ui_weak.clone();
        let state = state.clone();
        let notify = sync_notify.clone();
...
pub(crate) fn setup_toggle_product_addon_group(ui: &MainWindow) {
    let ui_weak = ui.as_weak();
    ui.global::<ProductsState>().on_toggle_product_addon_group(move |id| {
        let Some(ui) = ui_weak.upgrade() else { return };
        let model = ui.global::<ProductsState>().get_product_addon_groups();
    ...
    handle.spawn(async move {
        ...
        let _ = slint::invoke_from_event_loop(move || {
            if let Some(ui) = ui_weak.upgrade() {
                ui.global::<ProductsState>().set_product_addon_groups(ModelRc::new(VecModel::from(data)));
            }
        });
    });
}
