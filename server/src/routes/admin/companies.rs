...
pub(super) struct CompanyRow {
    ...
    next_charge: String,
    discount: String,
    /// Nome/rótulo do desconto (pré-preenche o modal de gerenciar).
...
pub(super) async fn create_company(
    ...
    let info = letaf_core::company::service::UpdateInfoInput {
        ...
        default_scheme: None, // idem — tema padrão definido nas Configurações
    };
...
pub(super) async fn update_company(
    ...
    let info = letaf_core::company::service::UpdateInfoInput {
        ...
        // Preserva o tema padrão escolhido pela empresa.
        default_scheme: current.default_scheme,
    };
...
pub(super) async fn list_companies(
    ...
    let base_domain = std::env::var("PUBLIC_BASE_DOMAIN").unwrap_or_else(|_| "letaf.app".into());
    for c in tenants {
        let (plan, plan_id, status, payment_kind, next_charge, discount, discount_name) = match by_company.get(&c.id) {
            Some(sub) => (
                sub.plan_name.clone(),
                ...
                sub.plan_discount_name.clone(),
            ),
            None => (String::new(), String::new(), "none".to_string(), String::new(), String::new(), "0".to_string(), String::new()),
        };
