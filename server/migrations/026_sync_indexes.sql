...
-- - Sem esses índices, sync escaneia tabela inteira a cada ciclo.
...
-- companies (sem company_id; tem id próprio + synced/updated_at)
...
-- customer_addresses (só existe no server)
CREATE INDEX IF NOT EXISTS idx_customer_addresses_company_synced
    ON customer_addresses(company_id, synced);
CREATE INDEX IF NOT EXISTS idx_customer_addresses_company_updated
